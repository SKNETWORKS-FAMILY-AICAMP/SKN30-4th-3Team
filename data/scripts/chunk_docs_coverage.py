"""생육 보충 normalized 문서를 기존 RAG 형식으로 청킹합니다.

용도
----
- 기존 ``chunk_documents.py``의 청크 구조, 메타데이터, 출처 생성,
  안전 태그, 청크 ID 생성 방식을 그대로 사용합니다.
- normalized 문서에 ``crop_or_plant``가 있으면 그 대표명을 그대로
  사용하고 본문에서 다른 식물명을 추가 추론하지 않습니다.
- ``crop_or_plant``가 비어 있는 일반 문서는 기존 방식대로 추론합니다.

국립수목원 생육 데이터 실행
---------------------------
python data/scripts/chunk_docs_coverage.py `
  --input data/interim/coverage_plant_grow_kna_api/normalized/kna_plant_growth.normalized.jsonl `
  --output data/processed/rag_chunks.kna_growth.jsonl `
  --sources-output data/processed/rag_sources.kna_growth.jsonl `
  --max-chars 1400 `
  --overlap-chars 160
...
python data/scripts/embed_chunks.py `
  --input data/processed/rag_chunks.kna_growth.jsonl `
  --output data/processed/rag_chunks.kna_growth.embedded.jsonl
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Chunk normalized coverage documents with citation metadata."
    )
...
def source_record(
    ...
    source = registry.get(doc.get("source_key")) or registry.get(
        doc.get("source_id")
    ) or {}
    return {
...
def chunk_record(
    ...
    doc_id = doc["doc_id"]
    chunk_key = (
        f"{source_key or source_id}:{stable_hash(doc_id, 10)}:{index:04d}"
    )
    chunk_id = uuid_for_chunk_key(chunk_key)
    safety_tags = merge_safety_tags(doc.get("safety_tags"))
    content = normalize_text(text)
    crop_or_plant = crop_or_plant_for_chunk(doc, content)
    symptom_keywords = doc.get("symptom_keywords") or detect_symptom_keywords(
        content
    )
...
def main() -> None:
    ...
    for doc in docs:
        if not doc.get("source_id"):
            raise ValueError(
                f"Missing source_id for doc_id={doc.get('doc_id')}"
            )
        sources.setdefault(
            doc["source_id"],
            source_record(doc, registry),
        )
